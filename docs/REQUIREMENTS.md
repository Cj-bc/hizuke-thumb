...
- [x] プロジェクト要件定義
- [ ] プロジェクトセットアップ（Vite + Svelte 5 + TypeScript + Bun）
- [ ] 基本UI構築（レイアウト、右サイドバー、ヘッダー）
- [ ] IndexedDB 実装（プリセット、画像、フォント）
- [ ] Canvas Engine 実装（基本描画）
- [ ] レイヤー管理（追加、削除、並び替え）
- [ ] 画像レイヤー（アップロード、表示）
- [ ] テキストレイヤー（静的テキスト、基本書式）
- [ ] プリセット保存・読み込み（自動保存 + 名前付け保存）
- [ ] PNG エクスポート

### Phase 2: テキスト機能

- [ ] フォント管理（アップロード、IndexedDB保存）
- [ ] テキスト詳細書式（アウトライン、シャドウ等）
- [ ] 動的テキスト（日付）
- [ ] 日付フォーマット（プリセット + カスタム + 言語選択）
- [ ] 複数日付選択機能
- [ ] 複数日付の一括生成・ZIP出力
